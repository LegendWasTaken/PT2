...
//                            reinterpret_cast<float *>(&best.emission), 3);
    ...
    void build_bvh() {
        // Generate the highest level AABB
        /*
        top_node = std::make_unique<BvhNode>();
        for (const auto &triangle : triangles) {
            top_node->aabb.expand(triangle.verts[2].position);
        }
        std::swap(triangles, top_node->triangles);

        auto recurse_build_node = [](BvhNode *node, auto &&recurse_build_node) {

            if (node->triangles.size() > 3) {
                constexpr auto bin_count = 8;
                ...
                for (uint8_t i = 0; i < 3; i++) {
                    const auto axis_bin_size = node->aabb.size_x() / bin_count;
                    for (uint8_t j = 0; j < bin_count; j++) {
                        const auto axis_offset = static_cast<float>(j) * axis_bin_size;
                        auto left_aabb = AABB{};
                        left_aabb.min_x = node->aabb.min_x;
                        left_aabb.min_y = node->aabb.min_y;
                        left_aabb.min_z = node->aabb.min_z;
                        left_aabb.max_x = i == 0 ? left_aabb.min_x + axis_offset : node->aabb.max_x;
                        left_aabb.max_y = i == 1 ? left_aabb.min_y + axis_offset : node->aabb.max_y;
                        left_aabb.max_z = i == 2 ? left_aabb.min_z + axis_offset : node->aabb.max_z;

                        auto right_aabb = AABB{};
                        right_aabb.min_x = i == 0 ? node->aabb.min_x + axis_offset : node->aabb.max_x;
                        right_aabb.min_y = i == 1 ? node->aabb.min_y + axis_offset : node->aabb.max_y;
                        right_aabb.min_z = i == 2 ? node->aabb.min_z + axis_offset : node->aabb.max_z;
                        right_aabb.max_x = node->aabb.max_x;
                        right_aabb.max_y = node->aabb.max_y;
                        right_aabb.max_z = node->aabb.max_z;

                        std::vector<Triangle> left;
                        std::vector<Triangle> right;
                        left.reserve(node->triangles.size() / 2);
                        right.reserve(node->triangles.size() / 2);

                        // Move the triangles into their bins
                        for (const auto &current : node->triangles) {
                            if (left_aabb.contains(current.aabb.get_center()))
                        ...
                        // Resize the AABB's
                        for (const auto &tri : left) left_aabb.expand(tri.aabb);
                        for (const auto &tri : right) right_aabb.expand(tri.aabb);

                        // Calculate the surface area cost
                        const auto current_sah = left_aabb.size_x() * left_aabb.size_y() * left_aabb.size_z() +
                                                 right_aabb.size_x() * right_aabb.size_y() * right_aabb.size_z();
                        // Minor optimization, we can get rid of actual surface area calculations, as long as we
                        // increase the more surface area there is, it'll still work.
                        if (current_sah < best_sah) {
                            std::swap(left, best_left);
                            std::swap(right, best_right);
                            std::swap(best_left_aabb, left_aabb);
                            std::swap(best_right_aabb, right_aabb);
                            best_sah = current_sah;
                        }
                    }
                }
                node->left = std::make_unique<BvhNode>();
                node->left->aabb = best_left_aabb;
                std::swap(node->left->triangles, best_left);
                node->right = std::make_unique<BvhNode>();
                node->right->aabb = best_right_aabb;
                std::swap(node->right->triangles, best_right);
                node->triangles.clear();
            }
        };

        recurse_build_node(top_node.get(), recurse_build_node);
         */
    }

    Model::Model(std::vector<Vec3> &vertices, std::vector<Vec3> &normals, std::vector<Indice> &indices,
                 std::vector<std::pair<float, float>> &textures) {
        std::swap(_vertices, vertices);
        std::swap(_normals, normals);
        std::swap(_indices, indices);
        std::swap(_textures, textures);
    }
